...
namespace University_Course_Registration_System
{
    ...
    public class Student
    {
        ...
        public bool CanAddCourse(Course course)
        {
            ...
            var i =RegisteredCourses.FirstOrDefault(c=>c.CourseCode==course.CourseCode);
            if(i!=null){
                return false;
            }
            if((GetTotalCredits()+course.Credits)>MaxCredits) return false;
            for(int j=0;j<course.Prerequisites.Count;j++){
                if(!CompletedCourses.Contains(course.Prerequisites[j])) return false;
            }
            return true;
        ...
        public void DisplaySchedule()
        {
            ...
            if(RegisteredCourses.Count<=0) {
                Console.WriteLine("Student has not registered in any course");
                return;
            }
            foreach(Course c in RegisteredCourses){
